...
const currentRole = computed(
    () =>
        (authStore.state.currentUser?.roleCode ??
            authStore.state.session?.userInfo.roleCode ??
            "STUDENT") as UserRoleCode
);
...
<template>
    <div class="user-page">
        <section class="hero-card">
            ...
            <h2>名下账号管理</h2>
            <p>超级管理员可管理所有人，管理员可管理教师，教师可管理自己名下的学生。</p>
        </section>
        ...
        <section class="toolbar-card">
            ...
            <div class="toolbar-actions">
                ...
                <ElButton v-if="roleOptions.length" type="success" @click="createDialogVisible = true"
                    >新增用户</ElButton
                >
            </div>
        </section>
                    ...
                    <ElTableColumn label="操作" min-width="300" fixed="right">
                        <template #default="{ row }">
                            <div class="action-row">
                                ...
                                <ElButton v-if="canManageStatus()" link type="danger" @click="handleResetPassword(row)"
                                    >重置密码</ElButton
                                >
                                <ElButton
                                    v-if="canDeleteStudent(row)"
                                    link
                                    type="danger"
            ...
            <ElForm ref="createFormRef" :model="createForm" :rules="createRules" label-position="top">
                ...
                <ElFormItem :label="createFieldLabel(selectedCreateRole)" prop="account">
                    <ElInput
                        ...
                        :placeholder="`请输入${createFieldLabel(selectedCreateRole)}`"
                    />
                </ElFormItem>
