...
async function handlePickup(row: BorrowRecordItem) {
    try {
        await ElMessageBox.confirm("确认领取后，系统会自动登记当前时间。", "确认领取", {
...
<template>
    <div class="borrow-page">
        <section class="hero-card">
            ...
            <h2>领取、归还与逾期跟踪</h2>
            <p>学生领取时不再手填时间，系统会自动登记当前领取时间；归还时间改为按分钟选择，和后端保持一致。</p>
        </section>
            ...
            <ElTable :data="page.list" v-loading="loading" width="100%">
                ...
                <ElTableColumn prop="status" label="状态" min-width="140">
                    <template #default="{ row }">
                        <ElTag
                            :type="
                                row.status === 'BORROWING' ? 'warning' : row.status === 'RETURNED' ? 'success' : 'info'
                            "
                        >
                ...
                <ElTableColumn label="设备状态" min-width="120">
                    <template #default="{ row }">
                        {{ deviceConditionLabel(row.deviceCondition) }}
                    </template>
                </ElTableColumn>
                <ElTableColumn label="操作" min-width="260" fixed="right">
                    <template #default="{ row }">
                        <div class="action-row">
                            <ElButton
                                v-if="isStudent && row.status === 'PICKUP_PENDING'"
                                link
                                type="primary"
                            ...
                            <ElButton
                                v-if="isStudent && (row.status === 'BORROWING' || row.status === 'OVERDUE')"
                                link
                                type="success"
                            ...
                            <ElButton
                                v-if="isAdmin && row.status === 'BORROWING'"
                                link
                                type="danger"
                                @click="handleMarkOverdue(row)"
                            >
                                标记逾期
                            </ElButton>
                        </div>
