...
import type { DeviceItem, DeviceListQuery, DeviceStatus, SaveDevicePayload } from "@/types/device";
...
const canDelete = computed(() => currentRole.value === "SUPER_ADMIN");
...
async function handleDelete(row: DeviceItem) {
    try {
        await ElMessageBox.confirm(`确认删除设备“${row.deviceName}”吗？该操作不可恢复。`, "删除设备", {
...
<template>
    <div class="device-page">
            ...
            <ElTable :data="page.list" v-loading="loading" width="100%">
                ...
                <ElTableColumn prop="status" label="状态" min-width="140">
                    <template #default="{ row }">
                        <ElTag
                            :type="
                                row.status === 'AVAILABLE' ? 'success' : row.status === 'DISABLED' ? 'info' : 'warning'
                            "
                        >
                ...
                <ElTableColumn label="操作" min-width="400" fixed="right">
                    <template #default="{ row }">
                        <div class="action-row">
                            ...
                            <ElDropdown
                                v-if="canManage"
                                @command="(status: DeviceStatus) => handleStatusChange(row, status)"
                            >
        ...
        <ElDialog
            ...
            :title="saveForm.deviceId ? '编辑设备' : '新增设备'"
            width="520px"
            @closed="resetSaveForm"
        >
