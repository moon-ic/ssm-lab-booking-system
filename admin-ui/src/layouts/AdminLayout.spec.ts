...
describe("AdminLayout", () => {
    ...
    it("renders role-aware navigation and exposes the profile entry in the header", async () => {
        ...
        expect(wrapper.find(".profile-entry").attributes("aria-label")).toBe("进入个人中心");

        await wrapper.find(".message-button").trigger("click");
        expect(pushMock).toHaveBeenCalledWith("/messages");

        await wrapper.find(".profile-entry").trigger("click");
